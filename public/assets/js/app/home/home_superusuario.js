...
const tabs = {
    "venda_bruta": ["Equipe", "Supervisor", "Vendedor", "CPF/CNPJ", "Venda", "Data BOV", "BOV"],
    "venda_instalada": ["Equipe", "Supervisor", "Vendedor", "CPF/CNPJ", "Venda", "Data BOV", "BOV", "OS"],
    "venda_agendamento": ["Equipe", "Supervisor", "Vendedor", "CPF/CNPJ", "Venda", "Agendamento", "BOV", "OS"],
    "venda_tratamento": ["Equipe", "Supervisor", "Vendedor", "CPF/CNPJ", "Venda", "Status tratamento", "Setor", "Retorno"],
    "venda_cancelada": ["Equipe", "Supervisor", "Vendedor", "CPF/CNPJ", "Venda", "Data BOV", "Status Tratamento"],
    "venda_migracao": ["Equipe", "Supervisor", "Vendedor", "CPF/CNPJ", "Venda", "Mig Fibra", "Mig Voip"],
    "venda_dados_incompletos": ["Equipe", "Supervisor", "Vendedor", "CPF/CNPJ", "Venda"],
    "venda_instalacao_atrasada": ["Equipe", "Supervisor", "Vendedor", "CPF/CNPJ", "Venda", "Ag", "R1", "R2", "R3", "OS"],
    "venda_bov": ["Equipe", "Supervisor", "Vendedor", "CPF/CNPJ", "Venda", "Data BOV", "OS"],
}
...
function carregaDados() {
    ...
    fetch(pag_url + 'carregaDados', {
        ...
        response.json().then(function (data) {
            ...
            document.querySelector("#vendas_emtratamento_atrasado").innerHTML = data.vendas_emtratamento_atrasado;
            ...
            if(document.querySelector("#vendas_emtratamento_atrasado").innerHTML>0){
                document.querySelector("#vendas_emtratamento_atrasado").classList.add("red");
            }
        });
    });
}
    ...
    fetch(pag_url + 'dadosModal', {
        ...
        response.json().then(function (data) {
            ...
            if (novo_titulo == "Vendas Brutas") {
                ...
                var varre_array = [...data.vendas_instalacoes_atrasadas].forEach((v, i) => {
                    tabela +=
                        "<tr><td>" + v.equipe + "</td><td>" + v.supervisor + "</td><td>" + v.vendedor +
                        "</td><td>" + v.cpf_cnpj_csv +
                        "</td><td>" + v.dt_venda_csv + 
						"</td><td>" + v.dt_agendamento + "</td><td>" + v.dt_reagendamento_1 + "</td><td>" + v.dt_reagendamento_2 + "</td><td>" + v.dt_reagendamento_3 + "</td><td>" 
						+ v.num_os + "</td></tr>";
                });
            }
            else if (novo_titulo == "Instaladas BOV") {                
                var tp_array = [...tabs.venda_bov];
                var varre_array = [...data.vendas_bov].forEach((v, i) => {
                    tabela +=
                        "<tr><td>" + v.equipe + "</td><td>" + v.supervisor + "</td><td>" + v.vendedor +
                        "</td><td>" + v.cpf_cnpj_csv +
                        "</td><td>" + v.dt_venda_csv + "</td><td>" + v.instalacao + "</td><td>" + v.numero_pedido + "</td></tr>";
                });
            }
            ...
            if(novo_titulo == "Em Tratamento"){
                ...
                $('#tableBasicas').DataTable({
                    "order": [[4, "asc"], [1, "desc"], [2, "desc"], [3, "desc"], [4, "desc"]],
                    "paging": false,
                    "info": false
                });
            }
...
function tableToCSV() {
    ...
    var csv_data = [];
	var rows_th = document.querySelectorAll('thead tr th');
	var csvrow = [];
	for (var j = 0; j < rows_th.length; j++) {
		// Get the text data of each cell
		// of a row and push it to csvrow
		csvrow.push(rows_th[j].innerHTML);
	}
	csv_data.push(csvrow.join(";"));
	//csv_data.push(rows_th.join(";"));
    // Get each row data
