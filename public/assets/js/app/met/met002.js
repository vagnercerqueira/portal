        ...
        .then(function (response) {
            response.text().then(function (data) {
                if (data === "") {
                    ...
                    instalacao.value = "";
                    tipo_meta.value = "";
                    error_alert("#form_meta", false, "Ainda não há tipo de meta definida para o mês selecionado", 3000);
                }
                else {
                    tipo.value = data;
                    if (data == 1) {
...
var pre_cadastrar_alterar = function (formu, dados) {
    let tipo_meta = document.querySelector('#TIPO_META');
    if (tipo_meta.value == "") {
        error_alert("#form_meta", false, "Voce não conseguir cadastrar, tipo de meta não definido", 3000);
        return false;
    }
    else {
        return dados;
