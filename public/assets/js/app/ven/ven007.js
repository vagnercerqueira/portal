//-------------------------TAB DFV------------------------------------
var tableDfvs;
const campanha = document.getElementById("ID_CAMPANHA");
const velocidade = document.getElementById("BANDA_LARGA_VELOCIDADE_CSV");
const plano_tv = document.getElementById("PLANO_TV_CSV");
...
//-------------------------TAB BRPRONTO-----------------------------------------------
/*const btn_download_brpronto = document.querySelector('.btn_download_brpronto'); 
btn_download_brpronto.addEventListener('mouseover', (e)=>{
	download_modelo(btn_download_brpronto, 'MODELO_brpronto');
})*/

document.getElementById('files_brpronto').addEventListener('change', (ele) => {
	valida_files_brpronto();
	limpa_relogio_brpronto();
});
function openDialogbrpronto() {
	document.getElementById('files_brpronto').click();
	limpa_relogio_brpronto();
}

document.querySelector('.cancel_brpronto').addEventListener('click', (ele) => {
	limpa_relogio_brpronto();
	document.querySelector(".table_files_brpronto tbody").innerHTML = "";
	document.getElementById('form_upload_brpronto').reset();
});

function limpa_tabela_brpronto() {
	document.querySelector(".table_files_brpronto tbody").innerHTML = "";
	document.getElementById('form_upload_brpronto').reset();
}
function limpa_relogio_brpronto() {
	document.querySelector(".timer_upload_ini_brpronto").innerHTML = "";
	document.querySelector(".timer_upload_fim_brpronto").innerHTML = "";
	document.querySelector('.row_msg_upload_brpronto').innerHTML = "";
}

function valida_files_brpronto(file) {
	let tab = document.querySelector(".table_files_brpronto tbody");
	tab.innerHTML = "";
	var fileInput = document.getElementById("files_brpronto");
	var files = fileInput.files;
	...
	[...files].forEach((file, ind) => {
		...
		if (!(['CSV', 'csv'].includes(formato))) {
			error_alert("#form_upload_brpronto", false, ((file.name) + ", formato de arquivo invalido. Aceita somente .csv "), 1000);
			document.getElementById('form_upload_brpronto').reset();
		} else {
...
const form_upload_brpronto = document.getElementById('form_upload_brpronto');
form_upload_brpronto.addEventListener('submit', uploadSubmitbrpronto);

async function uploadSubmitbrpronto(event) {
	event.preventDefault();
	var fileInput = document.getElementById("files_brpronto");
	var files = fileInput.files;
	...
	if (files.length > 0) {
		...
		document.querySelector('.timer_upload_ini_brpronto').innerHTML = "<i class='fas fa-hourglass'></i>&nbsp;Inicio: " + relogio();
		console.log('inicio sobe brpronto: ' + relogio());
		error_alert("#form_upload_brpronto", 'loading', 'Fazendo upload dos arquivos...!!!', 1000);
		let data = new FormData(document.getElementById('form_upload_brpronto'));
		fetch(pag_url + 'upload_brpronto', {
			method: 'POST',
			credentials: 'same-origin',
			body: data
		})
			...
			.then(function (resp) {
				toastr.clear();
				if (resp.error == 0) {
					error_alert("#form_upload_brpronto", true, ( 'Base vendas atualizada com sucesso,  '+(resp.aff)+' vendas atualizadas!!!' ), 1000  );
					document.querySelector(".table_files_brpronto tbody").innerHTML = "";
					document.getElementById('form_upload_brpronto').reset();
					...
					error_alert("#form_upload_brpronto", false, 'Erro ao processar: ' + (resp.message), 1000);
				}
			})
			.catch(function (error) {
				error_alert("#form_upload_brpronto", false, "Erro no servidor ao fazer upload brpronto ", 1000);
				toastr.clear();
			}).finally(function () {
				console.log('termino sobe brpronto: ' + relogio());
				document.querySelector('.timer_upload_fim_brpronto').innerHTML = "<i class='far fa-hourglass'></i>&nbsp;Termino: " + relogio();
			});
	} else {
		alert('obrigatorio o arquivo brpronto');
	}
}
...
var apos_cancelar = (formu) => {
	velocidade.innerHTML = "<option value=''>Selecione...</option>";
	plano_tv.innerHTML = "<option value=''>Selecione...</option>";
};
...
document.getElementById('PDF_AUDIT').addEventListener('change', (ele) => {
	let fileInput = document.getElementById("PDF_AUDIT")	
...
campanha.addEventListener('blur', (e) => {
	let idCampanha = campanha.value;
	busca_velocidade(idCampanha, "")
});

function busca_velocidade(idCampanha, vdefaul){
	
	velocidade.innerHTML = "<option value=''>Selecione...</option>";
	plano_tv.innerHTML = "<option value=''>Selecione...</option>";
	oData = new FormData();
	oData.append('id_campanha', idCampanha);
	fetch(
		pag_url + 'busca_velocidade', {
		method: 'POST',
		body: oData
	}).then(function (response) {
		response.json().then(function (data) {
			[...data].forEach( (idx, v)=>{
				...
				opt.value = idx.id;
				opt.text = idx.descricao;
				velocidade.add(opt);
				if( idx.id == vdefaul ) opt.setAttribute("selected", "");
			})
		});
	});	
}
...
velocidade.addEventListener('blur', (e) => {
	let idCampanha = campanha.value;
	let idVelocidade = velocidade.value;
	busca_tv(idCampanha, idVelocidade, 0)
});

function busca_tv(idCampanha, idVelocidade, vdefaul){
	plano_tv.innerHTML = "<option value=''>Selecione...</option>";
	
	oData = new FormData();
	oData.append('id_campanha', idCampanha);
	oData.append('id_velocidade', idVelocidade);
	fetch(
		pag_url + 'busca_tv', {
		method: 'POST',
		body: oData
