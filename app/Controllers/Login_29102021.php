...

{
                    ...
                    if (count($usu) > 0) {
                        if ((password_verify($dPost['senha'], $usu[0]['senha'])) || (password_verify($dPost['senha'], $usu[0]['senha_temp']))) {
                            if ($usu[0]['status'] != 'A') {
                                ...
                                $ddUser = array_merge(["logado" => true], $usu[0], ["cliente_db" => $dPost['cliente_db']], ['logo_cliente' => $banco[0]['logo']]);
                                $ses->set($ddUser);
    ...
    public function clientes_permitidos($dPost)
    {
        ...
        $sql = "SELECT nome, status, logo FROM clientes WHERE codigo_db=? ";
        $clis = $db_cpanel->query($sql, [$dPost['cliente_db']])->getResultArray();
        return $clis;
    }
