...
class Ven007 extends BaseController
{
    ...
    private $campos_brpronto = ["Protocolo","CPF/CNPJ","N. da Linha","Nº do Contrato","Nº do Voucher","3G Substituto",
								"Mudinho","Nº IMEI","Nº IDGPON Acesso","Nº da O.S","Nº ID Bundle","Linha Provisória",
								"Nº ICCID NOVO","Data de Digitalização","Data de Envio da Digitalização","Data de Conferência",
								"Regional","Tipo de Loja","Nome Fantasia","Código PDV","Login","Tipo de Serviço",
								"Origem da Digitalização","Resultado da Análise"];
    ...
    /*private $campos_venda_lote = [  'DT VENDA', 'NOME CLIENTE', 'GENERO', 'DT NASCIMENTO', 'CPF/CNPJ', 'RG',
                                    ...
                                    'CEP', 'LOGRADOURO', 'NÃšMERO', 'BAIRRO', 'CIDADE', 'UF', 'REFERENCIA', 'COMPLEMENTO1 TIPO',
...

    {
		...
		$this->campanhas_model = new Campanhas_Model();
    ...
    public function index()
    {
		...
		$campanhas = ['' => 'Selecione...'] + array_column($this->campanhas_model->findAll(), 'descricao', 'id');
        ...
        $data = [
            ...
            "planos_fibra" => form_dropdown('BANDA_LARGA_VELOCIDADE_CSV', ['' => 'Selecione...'], '', "id='BANDA_LARGA_VELOCIDADE_CSV' class='form-control form-control-sm' required"),
            "planos_tv" => form_dropdown('PLANO_TV_CSV', ['' => 'Selecione...'], '', "id='PLANO_TV_CSV' class='form-control form-control-sm'"),
			
			"campanhas" => form_dropdown('ID_CAMPANHA', $campanhas, '', "id='ID_CAMPANHA' class='form-control form-control-sm' required "),
		...
		"option_cols_dfv"=>$this->campos_brpronto
        ];
    ...
    public function upload_dfv()
    {
        ...
        if (strtoupper($tipo) ==  '.ZIP') {
            ...
            if ($tot_csvs == 1) {
                ...
                $this->resposta_upload = ["error" => 1, "message" => "Nao Ã© permitido mais de uma arquivo csv!!!"];
            }
        } else {
 ...
 //------------------------------------BRPRONTO---------------
    public function upload_brpronto()
    {

        $this->db = db_connect();
	
        $files = $_FILES['files_brpronto'];

        $this->db->transStart();
		$campos_brpronto = $this->campos_brpronto;		
        ...
        foreach ($files['tmp_name'] as $m => $n) {
            ...
            $IdxCamposInsert = array_intersect($csv_head, $campos_brpronto);			

			$head_valid = true;
			foreach( array_values($campos_brpronto) as $kc=>$vc ){ 
				//if( remove_acentos(addslashes($vc)) != remove_acentos(addslashes(utf8_encode($csv_head[$kc]))) ){
				if( trim( preg_replace("/[^a-zA-Z0-9]+/", "", $vc)) != trim( preg_replace("/[^a-zA-Z0-9]+/", "", $csv_head[$kc])) ){
            ...
            if (count($campos_brpronto) != count($csv_head) || $head_valid === false) {//quantidade de colunas incorretas
                ...
                while (($data = fgetcsv($csv_handle, 5000, ";")) !== FALSE) {
                    $reg++;
                  //  if ($reg == 1) continue;				
					$result_analise = $data[23];
					if( $result_analise == "Doc. Apto para Venda" ){
						$cpfCnpj = trim($data[1]);
						$rows_vendas = $this->db->query("SELECT id, dt_venda_csv FROM vendas WHERE cpf_cnpj_csv='{$cpfCnpj}'")->getResultArray();
						if(count($rows_vendas) > 0){
							$dtDigitArr = explode("/",substr($data[13], 0, 10));					
							$dtDigit_minus = date('Y-m-d', strtotime( ($dtDigitArr[2]."-".$dtDigitArr[1]."-".$dtDigitArr[0]). " -15 days" ));
							foreach($rows_vendas as $key=>$val){								
								$iniDate  = new DateTime($dtDigit_minus);
								$fimDate = new DateTime($val['dt_venda_csv']);								
								$difDias = $iniDate->diff($fimDate)->d;
								if($difDias <= 15){
									$this->db->query(" UPDATE vendas SET status_brpronto = 1 WHERE id = '{$val['id']}'");
        ...
        $this->db->transComplete();
		$this->resposta_upload['aff'] = $aff;
        if($aff > 0) $this->envio_email("brpronto");
        echo json_encode($this->resposta_upload);
    }
    //------------------------------------BOV---------------
        ...
        foreach ($files['tmp_name'] as $m => $n) {
            ...
            if (count($campos_bov) != count($csv_head) || $head_valid === false) {//quantidade de colunas incorretas
                ...
                while (($data = fgetcsv($csv_handle, 5000, ";")) !== FALSE) {
                    ...
                    if ($arr_insert['STATUS'] == "Concluido") { //status_ativacao vem da tabela status ativacoes(id), caso esteja instalada, muda a venda para tratado
						
                        $sqlUpdVendaAtiv = "UPDATE vendas SET status_ativacao = 5, status_tratamento = 'T'
                                        WHERE num_os = '{$arr_insert['NUMERO_PEDIDO']}'";
    ...
    public function envio_email($csv=""){
        ...
        $rows = $db->query($sqlGrpEnvio)->getResultArray();
		
        if(count($rows)> 0){
            helper('envia_email_helper');
            $emails = array_column($rows, 'email');
				$titulo = 'Atualizacao de base '. strtoupper($csv). " - ".date("d/m/Y h:i");
				$msg = "a base de ".strtoupper($csv)." foi atualizada, verifique o sistema: ".base_url("portal");
	...
	public function upload_venda_lote()
    {
        ...
        foreach ($files['tmp_name'] as $m => $n) {
            ...
            if (count($campos_dif)) {
                ...
                while (($data = fgetcsv($csv_handle, 5000, ",")) !== FALSE) {
                    if ($reg == 1) continue;
                    #A query abaixo nÃ£o permite a inclusÃ£o por lote, de um cliente que jÃ¡ tenha CPF na base.
					...
					//CASO NAO EXISTA O BANCO CADASTRADO NA BASE DE DADOS, IRA INSERIR AUTOMATICAMENTE
					$id_banco = array_search($arr_insert["pag_banco_csv"], $arr_bancos);
					if($id_banco == false){
						if( $arr_insert["pag_banco_csv"] == "-" || $arr_insert["pag_banco_csv"] == "" ){
							$id_banco = "null";
						}else{
							$id_banco = $this->bancos_Model->insert(['descricao'=>$arr_insert["pag_banco_csv"]]);
						}						
					}
					
					$arr_insert["pag_banco_csv"] = $id_banco;
    ...
    public function DataTable()
    {
        ...
        $sql = "SELECT
                    ...
                    END zap_4,
                    CONCAT(COALESCE(audio_audit_quality_1, ''), '|', 
					       COALESCE(A.audio_audit_quality_2,''), '|', 
					       COALESCE(A.pdf_audit,'')
						   ) auditoria,
        ...
        $dt = new Datatables_server_side([
            ...
            'formata_coluna' => [
                ...
                1 => function ($col, $lin) {
					...
					$auditoria_pdf = $this->auditoria($coluna[2], "fa-file-pdf", "btn btn-danger btn-xs");
					

                    return  $auditoria_1 . "&nbsp;&nbsp;" . $auditoria_2."&nbsp;&nbsp;".$auditoria_pdf;                },
                2 => function ($col, $lin) {
                    $id = base64_encode($lin['id']);
                    if ($col == 1) {
    ...
    public function auditoria($audi, $ico="fa-download", $classe = "btn btn-primary btn-xs")
    {
        $auditoria = ($audi != '' ? "<a href='" . base_url("ven/ven007/download?arquivo=" . $audi) . "' class='".$classe."' target='_blank' download>
                    <i class='fas {$ico}'></i></a>" : '');
        return $auditoria;
    }
	...
	$this->grava_auditoria($dados["CPF_CNPJ_CSV"], date('d_m_Y_H_i_s'), $_FILES['PDF_AUDIT'], $dados['ID'], 'pdf_audit');
    }
	...
	$this->grava_auditoria($f_dados["CPF_CNPJ_CSV"], date('d_m_Y_H_i_s'), $_FILES['PDF_AUDIT'], $key, 'pdf_audit');
    }
    public function preInsUpd($dados)

    {
		#Regra serve caso o status de ativacao for diferente de ATIVADO, 
        #e nÃ£o houve ajuste anteriormente pelo BKO, o sistema jÃ¡ coloca automaticamente em tratamento, com retorno para o dia seguinte.
        if($dados["STATUS_ATIVACAO"] != 5){
            $this->db = db_connect();
             $sql = "SELECT edit_usuario_id FROM vendas WHERE id = '{$dados['ID']}'";
			 //print_r(($this->db->query($sql)->getResultArray()[0]["edit_usuario_id"]); exit;
            if($this->db->query($sql)->getResultArray()[0]["edit_usuario_id"] == ""){ 
                $dados["STATUS_TRATAMENTO"] = "ET";
                $dados["SETOR_RESP_TRATAMENTO"] = 4;
                $dados["DT_RETORNO_TRATAMENTO"] = date('Y-m-d', strtotime("+1 day"));
            }
        }
    ...
    public function valida_campos()
    {
        $rules = [
            ...
            'DT_ATIVACAO'    => [
                ...
                'errors' => [
                    'valid_date' => 'Data ativaÃ§Ã£o invalida.'
                ]
            ],
            /*'DT_NASCIMENTO_CSV'    => [
                'rules'  => 'valid_date',
            ...
            'NUM_OS'    => [
                ...
                'errors' => [
                    'is_unique' => 'JÃ¡ existe este numero de OS.'
                ]
            ],
			'STATUS_ATIVACAO'    => [
                'rules'  => 'required',
    ...
    public function BuscaFaturamento()
    {
		...
		//print_r(number_format($busca_faturamento, 2, ",", ".")); exit;
		$faturamento = ($busca_faturamento != '' ? number_format($busca_faturamento, 2, ",", ".") : '');
        echo json_encode(['faturamento' => $faturamento]);
    }
    ...
    public function gerar_pdf()
    {
        ...
        $status_tratamento = ["T" => "TRATADO", "ET" => "EM TRATAMENTO", "NT" => "NÃƒO TRATADO", "" => ""];
        ...
        $pdf->Cell(136, 6, 'AUDITORIA:', 0, 0, 'R');
        $pdf->Cell(10, 6, $auditoria, 0, 1, 'L');
        $pdf->Cell(30, 6, 'BOV:', 0, 0, 'L');
        $pdf->Cell(133, 6, strtoupper(utf8_decode($rows->status_bov)), 0, 0, 'L');
		  $pdf->Cell(18, 6, 'BRPRONTO:', 0, 0);
        $pdf->Cell(20, 6, strtoupper(utf8_decode($rows->status_doc)), 0, 1);
       
        $pdf->Cell(30, 6, 'TRATAMENTO:', 0, 0);
        $pdf->Cell(20, 6, strtoupper(utf8_decode($status_tratamento[$rows->status_tratamento])), 0, 0);
		
		 $pdf->Cell(131, 6, 'WHATSAPP:', 0, 0, 'R');
        $pdf->Cell(18, 6, strtoupper(utf8_decode($rows->whatsapp)), 0, 1, 'L');
		
        $pdf->Cell(30, 6, 'RET TRATAMENTO:', 0, 0);
        $pdf->Cell(20, 6, $dt_retorno_tratamento, 0, 1);
        #Terceiro bloco
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Cell(95, 6, utf8_decode('SERVIÃ‡O CONTRATADO'), 0, 1);
        $pdf->SetFillColor(192, 192, 192);
        $pdf->Cell(195, 0.5, '', 0, 1, 'L', 1);
        $pdf->SetFont('Arial', '', 8);
        ...
        $pdf->Cell(22, 6, utf8_decode('NÂº FACHADA:'), 0, 0, 'R');
        ...
        $pdf->Cell(22, 6, utf8_decode('REFERÃŠNCIA:'), 0, 0, 'L');
        ...
        $pdf->Cell(195, 0.5, '', 0, 1, 'L', 1);
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(30, 6, utf8_decode('1Âº AGENDAMENTO:'), 0, 0, 'L');
        ...
        $pdf->Cell(30, 6, utf8_decode('2Âº AGENDAMENTO:'), 0, 0, 'L');
        ...
        $pdf->Cell(30, 6, utf8_decode('3Âº AGENDAMENTO:'), 0, 0, 'L');
        ...
        $pdf->Cell(30, 6, utf8_decode('4Âº AGENDAMENTO:'), 0, 0, 'L');
        ...
        $pdf->Cell(132, 6, utf8_decode('AGÃŠNCIA:'), 0, 0, 'R');
        ...
        #PAGINA E BLOCO PARA AS OBSERVAÃ‡Ã•ES
        //Obsercao vendedor
        $pdf->AddPage('P');
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Cell(95, 6, utf8_decode('OBSERVAÃ‡ÃƒO VENDEDOR'), 0, 1);
        $pdf->SetFillColor(192, 192, 192);
        $pdf->Cell(195, 0.5, '', 0, 1, 'L', 1);
        $pdf->SetFont('Arial', '', 8);
        $pdf->ln(3);
        ...
        //Obsercao supervisor
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Cell(95, 6, utf8_decode('OBSERVAÃ‡ÃƒO SUPERVISOR'), 0, 1);
        $pdf->SetFillColor(192, 192, 192);
        $pdf->Cell(195, 0.5, '', 0, 1, 'L', 1);
        $pdf->SetFont('Arial', '', 8);
        ...
        //Obsercao bko
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Cell(95, 6, utf8_decode('OBSERVAÃ‡ÃƒO TRATAMENTO BACKOFFICE'), 0, 1);
        $pdf->SetFillColor(192, 192, 192);
        $pdf->Cell(195, 0.5, '', 0, 1, 'L', 1);
        $pdf->SetFont('Arial', '', 8);
        ...
        //Obsercao bko
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Cell(95, 6, utf8_decode('OBSERVAÃ‡ÃƒO ATIVAÃ‡ÃƒO BACKOFFICE'), 0, 1);
        $pdf->SetFillColor(192, 192, 192);
        $pdf->Cell(195, 0.5, '', 0, 1, 'L', 1);
        $pdf->SetFont('Arial', '', 8);
    ...
    public function mquery($id)
    {
        ...
        $sql = "SELECT
                    ...
                    F.descricao tv,
					CASE
						WHEN COALESCE(A.status_brpronto) = 0 THEN 'REPROVADO'
						WHEN COALESCE(A.status_brpronto) = 1 THEN 'APROVADO'
						WHEN COALESCE(A.status_brpronto) = '' THEN 'N ENCONTRADO'
					END status_doc,
    ...
    public function sqlExportarvendas($data_inicial, $data_final)
    {
        ...
        if ($cod_interno == 9999 || $cod_interno == 1000) {
            ...
            $cabecalho = [
                ...
                "Velocidade", "TV", "BRPRONTO", "Status BOV", "Status Tratamento", "Responsavel", "Retorno", "Situacao Ativacao", "MIG FIB", "MIG VOIP",
        ...
        $sql = "SELECT
					...
					CASE
						WHEN COALESCE(A.status_brpronto) = 0 THEN 'REPROVADO'
						WHEN COALESCE(A.status_brpronto) = 1 THEN 'APROVADO'
						WHEN COALESCE(A.status_brpronto) = '' THEN 'NAO ENCONTRADO'
					END status_doc,

                    A.status_bov,
                    CASE
                        WHEN A.status_tratamento = 'ET' THEN 'EM TRATAMENTO'
                        WHEN A.status_tratamento = 'T'  THEN 'TRATADO'
                        WHEN A.status_tratamento = 'NT' THEN 'NAO TRATADO'
                    END status_tratamento,
                    L.descricao,
                    DATE_FORMAT(A.dt_retorno_tratamento, '%d/%m/%Y'),
                    C.descricao status_ativacao,
            ...
            LEFT JOIN setor_tratamento_vendas L ON (L.id = A.setor_resp_tratamento)
            WHERE
            $where_date
            $where_complemento";
	...
	public function busca_velocidade(){
		$this->db = db_connect();
		$id_campanha = $this->request->getPost('id_campanha');
		
		$sql = "SELECT A.descricao, A.id FROM plano_fibra A
				INNER JOIN planos_operadora B ON B.id_fibra=A.id
				WHERE B.id_campanha='{$id_campanha}'				
				order by A.descricao asc";
		
		$fibras = $this->db->query($sql)->getResultArray();
		echo json_encode($fibras);
	}
	
	public function busca_tv(){
		$this->db = db_connect();
		$id_campanha = $this->request->getPost('id_campanha');
		$id_velocidade = $this->request->getPost('id_velocidade');
		
		$sql = "SELECT C.descricao, C.id 
				FROM plano_fibra A
				INNER JOIN planos_operadora B ON B.id_fibra=A.id
				INNER JOIN plano_tv C ON C.id=B.id_tv
				WHERE B.id_campanha='{$id_campanha}' AND B.id_fibra='{$id_velocidade}'
				order by A.descricao asc";
