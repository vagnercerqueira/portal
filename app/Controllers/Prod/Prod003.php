...
class Prod003 extends BaseController
{
    ...
    public function producao(){
        ...
        $dados_vendedor = $this->monta_query_vendedor( $dPost['COMPETENCIA'], $dPost['EQUIPE'], $dPost['TIPO'] );
    ...
    {        
        ...
        if (count($row) > 0) {
            ...
            if ($row[0]["tipo"] == 2) { //1 = unitario 2 = faturamento
                $query_complemento = " SELECT SUM(E.faturamento) FROM vendas E WHERE E.id_vendedor = A.id AND DATE_FORMAT(dt_venda_csv, '%Y-%m') ='{$competencia}' {$where2}";
            } else {
                $query_complemento = " SELECT COUNT(E.id) FROM vendas E WHERE E.id_vendedor = A.id AND DATE_FORMAT(dt_venda_csv, '%Y-%m') ='{$competencia}' {$where2}";
            }
        } else {
            $query_complemento = " SELECT COUNT(E.id) FROM vendas E WHERE E.id_vendedor = A.id AND DATE_FORMAT(dt_venda_csv, '%Y-%m') ='{$competencia}' {$where2}";
        }
        ...
        $sql = "SELECT 
                        ...
                        COALESCE((SELECT {$instalacao_bruto} FROM meta_vendedor D WHERE D.equipe = C.id AND DATE_FORMAT(competencia, '%Y-%m') ='{$competencia}'),0) meta,
                        COALESCE(($query_complemento),0) acumulado,
                        FORMAT(COALESCE((COALESCE(($query_complemento),0)/COALESCE((SELECT {$instalacao_bruto} FROM meta_vendedor D WHERE D.equipe = C.id AND DATE_FORMAT(competencia, '%Y-%m') ='{$competencia}'),0)),0)*100,2) atingido,
                        ...
                        (((COALESCE(($query_complemento),0)/
                        (SELECT SUM(B.peso) FROM peso_dias B WHERE DATE_FORMAT(data, '%Y-%m') ='{$competencia}' AND B.data < CURRENT_DATE))*
                        (SELECT SUM(A.peso) trabalho FROM peso_dias A WHERE DATE_FORMAT(data, '%Y-%m') ='{$competencia}'))/COALESCE((SELECT {$instalacao_bruto} FROM meta_vendedor D WHERE D.equipe = C.id AND DATE_FORMAT(competencia, '%Y-%m') ='{$competencia}'),0))*100,0)
                        ,2) tend_rel,        
                        FORMAT(
                            (
                            COALESCE((SELECT {$instalacao_bruto} FROM meta_vendedor D WHERE D.equipe = C.id AND DATE_FORMAT(competencia, '%Y-%m') ='{$competencia}'),0) - 
                            COALESCE(($query_complemento),0))/
                            COALESCE(
                        ...
                        FORMAT(
                            (
                            COALESCE(((SELECT {$instalacao_bruto} FROM meta_vendedor D WHERE D.equipe = C.id AND DATE_FORMAT(competencia, '%Y-%m') ='{$competencia}')/100)*80,0) - 
                            COALESCE(($query_complemento),0))/
                            COALESCE(
                                (SELECT SUM(A.peso) trabalho FROM peso_dias A WHERE DATE_FORMAT(data, '%Y-%m') ='{$competencia}') - 
                                (SELECT SUM(B.peso) FROM peso_dias B WHERE DATE_FORMAT(data, '%Y-%m') ='{$competencia}' AND B.data < CURRENT_DATE),0)
    ...
    public function monta_query_vendedor($mes = "", $equipe=0, $tipo="B")
    {
