...

{
    ...
    public function index()
    {
        ...
        if ($metodo !== "post") {
            ...
            $rules = [
                'usuario'    => 'required',
                'senha'      => 'required',
                'cliente_db' => 'required|alpha_dash',
            ];
            ...
            if (!$this->validate($rules)) {
                ...
                $banco = $this->clientes_permitidos($dPost);
				
                if (count($banco) == 0) {
                    $arr_valida['error'] = "Codigo incorreto!!!";
                } else {

                    if ($banco[0]['status'] != 'A')  return view('pcliente');

					$base_cliente = $this->custom_db($banco[0]['codigo_db']);
					
                    //$db = db_connect();
					$db = \Config\Database::connect($base_cliente);
                    ...
                    if (count($usu) > 0) {
                        if ((password_verify($dPost['senha'], $usu[0]['senha'])) || (password_verify($dPost['senha'], $usu[0]['senha_temp']))) {
                            if ($usu[0]['status'] != 'A') {
								...
								$ses = session();
								$ses->set(["cliente_db"=>$banco[0]['codigo_db']]);
                                $arr_valida['valido'] = true;
								
                                if (password_verify($dPost['senha'], $usu[0]['senha_temp'])) $db->query("UPDATE usuarios SET senha='{$dPost['senha']}' WHERE id={$usu[0]['id_usuario']}");
                                
								$db->query("UPDATE senha_email SET status='I' WHERE usuario={$usu[0]['id_usuario']}");
								
                                unset($usu[0]['senha'], $usu[0]['senha_temp'], $usu[0]['status']);

                                $ddUser = array_merge(["logado" => true], $usu[0], ['logo_cliente' => $banco[0]['logo']]);
                                $ses->set($ddUser);

                                if ($usu[0]['qtd_acessos'] > 1) $db->query("INSERT INTO auditar_log (id_user,tb,acao) VALUES ({$usu[0]['id_usuario']},'usuarios', 'login')");
    ...
    public function clientes_permitidos($dPost)
    {		
		
        $db_cpanel = db_connect('db_cpanel');
        $sql = "SELECT nome, status, logo, codigo_db FROM clientes WHERE codigo_db=? or codigo_cliente=?";
        $clis = $db_cpanel->query($sql, [$dPost['cliente_db'], $dPost['cliente_db']])->getResultArray();
		
		//var_dump(validaBancoDados($clis[0]['codigo_db']));exit;
